...
selected_vars <- c(
  ...
  "DON_CARDIAC_ARREST_AFTER_DEATH", "DON_NON_HR_BEAT", "DON_TY",
  ...
  map_dfr(names(df), function(v) {
    ...
    if (is.numeric(vec)) {
      ...
      tibble(
        variable = v,
        type     = class(vec)[1],
        valid    = sprintf("%d (%s)", valid_n, fmt_pct(valid_pct)),
      ...
      levels_txt <- cat_summary(vec, top_n = top_n_levels)
      tibble(
        variable = v,
        type     = class(vec)[1],
        valid    = sprintf("%d (%s)", valid_n, fmt_pct(valid_pct)),
...
html_table <- summary_tbl |>
  kable("html", escape = FALSE, align = "lcccl",
        col.names = c("Variable", "Type", "Valid", "Missing", "Stats / Top Levels")) |>
  kable_styling(full_width = TRUE,
                bootstrap_options = c("striped", "hover"))

# ========== ADD HERE : header block  =====================================
ds_name   <- "tx_ki_selected"                     # dataset name
dim_line  <- sprintf("%d x %d",
                     nrow(tx_ki_selected),
                     ncol(tx_ki_selected))

header_block <- htmltools::tagList(
  htmltools::tags$h2("Data Frame Summary"),
